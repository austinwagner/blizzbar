...
namespace {
	...
	{
		auto view = MapViewOfFile(mmap, FILE_MAP_READ, 0, 0, len == 0 ? sizeof(T) : len);
		if (view == nullptr) throw std::exception();
		return reinterpret_cast<T*>(view);
	}
}

ConfigFileMapping::ConfigFileMapping() : m_mmapHandle(nullptr) { }

ConfigFileMapping::ConfigFileMapping(const wchar_t* name) 
	: m_mmapHandle(OpenFileMappingW(FILE_MAP_READ, false, name))
{
	if (m_mmapHandle == nullptr) throw std::exception();
}
...
ConfigFileMapping& ConfigFileMapping::operator=(ConfigFileMapping&& other) noexcept
{
	if (this != &other)
	{
		if (m_mmapHandle != nullptr) CloseHandle(m_mmapHandle);
		m_mmapHandle = other.m_mmapHandle;
		other.m_mmapHandle = nullptr;
	}
...
ConfigFileView::~ConfigFileView() { if (m_view != nullptr) UnmapViewOfFile(m_view); }

const Config* ConfigFileView::operator->() const { return reinterpret_cast<Config*>(m_view); }
const Config& ConfigFileView::operator*() const { return *reinterpret_cast<Config*>(m_view); }
