...
public:
    static std::unique_ptr<WindowPropStore> create(HWND window)
    {
        ...
        if (SHGetPropertyStoreForWindow(
                window, IID_IPropertyStore, reinterpret_cast<void**>(&p))
            != S_OK) {
            return nullptr;
        }
    ...
    explicit WindowPropStore(IPropertyStore* p)
        : m_propStore(p)
    {
    }

    ~WindowPropStore() { m_propStore->Release(); }
    WindowPropStore(const WindowPropStore& other)
    ...
    WindowPropStore(WindowPropStore&& other) noexcept
        : m_propStore(other.m_propStore)
    {
...
void SetWindowProps(HWND window)
{
    ...
    propStore->setString(
        PKEY_AppUserModel_RelaunchCommand, g_gameInfo->relaunchCommand);
    propStore->setString(
        PKEY_AppUserModel_RelaunchDisplayNameResource, g_gameInfo->gameName);
...
void ShellHookHandlerInternal(int code, WPARAM wparam, LPARAM /*unused*/)
{
    switch (code) {
...
{
    const auto exeNameNoExt
        = ((std::wstring_view)exeName).substr(0, exeName.size() - 4);
...
BOOL CALLBACK EnumThreadWindowsCallback(HWND hwnd, LPARAM /*unused*/)
{
...
BOOL WINAPI GetMessageDetourW(
    LPMSG msg, HWND hwnd, UINT msgFilterMin, UINT msgFilterMax)
{
...
void CheckIfProcessOfInterest()
{
    ...
    if (g_gameInfo != nullptr) {
        ...
        MinHookTry(MH_CreateHook(reinterpret_cast<void*>(&GetMessageW),
            reinterpret_cast<void*>(&GetMessageDetourW),
            reinterpret_cast<void**>(&g_origGetMessageProc)));
        ...
        // Should be plenty of time for all of the threads to enumerate their
        // windows
        Sleep(10000);
...
extern "C" __declspec(dllexport) LRESULT
    ShellHookHandler(int code, WPARAM wparam, LPARAM lparam)
{
...
DWORD WINAPI OnAttached(LPVOID /*unused*/)
{
    try {
...
BOOL WINAPI DllMain(HANDLE /*unused*/, DWORD reason, LPVOID /*unused*/)
{
    switch (reason) {
