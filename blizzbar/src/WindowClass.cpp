...
WindowClass::WindowClass(HINSTANCE inst, WNDPROC wndProc, std::wstring name)
    : m_name(std::move(name))
{
    m_class.hInstance = inst;
...
{
    if (m_class.lpszClassName == nullptr) {
        return;
    }
...
std::wstring WindowClass::className() const { return m_name; }
