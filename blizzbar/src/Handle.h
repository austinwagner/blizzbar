...
public:
    ...
    operator NativeHandle() const { return m_handle; }

    NativeHandle get() const { return m_handle; }
...
struct Window : Handle<HWND, WindowDeleter> {
    Window(DWORD extendedStyle, const std::wstring& className,
...
struct FileMapping : Handle<HANDLE, FileMappingDeleter> {
    FileMapping(HANDLE file, LPSECURITY_ATTRIBUTES attributes, DWORD protection,
        DWORD maxSize, const std::wstring& name);
};
...
struct Mutex : Handle<HANDLE, MutexDeleter> {
    ...
    static Mutex create(LPSECURITY_ATTRIBUTES attrs, bool initialOwner,
...
struct RegistryKey : Handle<HKEY, RegistryKeyDeleter> {
    ...
    static RegistryKey create(HKEY baseKey, const std::wstring& subKeyPath,
